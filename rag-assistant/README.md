...
1. 有截图时先做视觉摘要，让截图中的报错文字与用户输入一起参与判断；
2. 使用廉价的语义闸门分流：只有明确无关的输入会在查询 KB 前直接拒绝；明确报错和暂时无法判断的输入继续查询 KB，避免误挡未知报错；
3. 检索飞书知识库，按文档段落切分，并用语义相似度与关键词混合召回；
4. KB 命中时直接回答；KB 未命中时，只有语义闸门明确判为报错问题才允许只读扫描源码；
5. 语义判断不确定或服务异常时禁止扫描源码，并提示用户补充错误文本、错误码或清晰截图；源码分析产生的中高置信度结论写入飞书待审核池，人工审核后才进入正式知识库。
...
- KB 按飞书文档里的 `##` 条目完整切分，不再按固定字数截断；一个错误条目的标题、症状、原因、解决方案和关键词会一起进入索引。
- 查询使用“语义相似度 + 关键词”混合排序。错误码、英文报错短语和 KB 明确关键词会优先召回；只有语义沾边但没有共同关键词的低分条目会被过滤，减少把无关内容交给模型造成幻觉。
- 默认参数是 `top_k=5`、候选 `20`、向量最低分 `0.52`、混合最低分 `0.38`，可在 `state/secrets.json` 调整。修改切分方式后必须重新执行一次 `/admin/reindex`，让新 KB 重新建立索引。
