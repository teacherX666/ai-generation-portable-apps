...
class PortalNavStructureTests(unittest.TestCase):
    ...
    def test_every_nav_tab_has_a_matching_panel(self):
        html = self._read()
        nav_tabs = re.findall(r'class="app-tab[^"]*"[^>]*data-tab="([^"]+)"', html)
        panel_ids = set(re.findall(r'<(?:div|section)\b[^>]*\bid="(tab-[^"]+)"', html))
        for tab in nav_tabs:
            self.assertIn(f"tab-{tab}", panel_ids, f"missing panel for {tab}")

    def test_home_is_first_nav_entry(self):
        html = self._read()
        first = re.search(r'<button[^>]*class="app-tab[^"]*"[^>]*data-tab="([^"]+)"', html)
        self.assertIsNotNone(first)
        self.assertEqual(first.group(1), "home")
        self.assertIn('id="portalHomeBtn"', html)

    def test_help_entries_exist(self):
        html = self._read()
        self.assertIn('id="helpBtn"', html)
        self.assertIn('id="moduleHelpBtn"', html)
